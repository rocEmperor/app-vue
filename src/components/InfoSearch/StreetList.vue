<template>
  <div class="search-own">
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>街道查询</el-breadcrumb-item>
    </el-breadcrumb>
    <el-card class="box-card">
      <el-form ref="searchForm" :model="model.searchList">
        <el-row type="flex" class="row-bg">
          ...
          <el-col :span="6" style="padding-right: 10px">
            <el-form-item label="是否党员：" label-width="76px">
              <el-select v-model="model.searchList.is_party" placeholder="请选择">
                <el-option
                  v-for="item in model.party_options"
                  :key="item.value"
                  :label="item.label"
                  :value="item.value">
                </el-option>
...
<script>
  ...
  export default {
    name: 'SearchOwn',
    computed: mapState({
      model: state => state.StreetListStore
    }),
    created: function () {
      this.$store.dispatch({
        type: 'StreetListStore/reqData',
        payload: {
